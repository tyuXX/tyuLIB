...
class LargeNumber extends DerivedNumberType {
    ...
    add(other) {
        ...
        const sum = (this.sign * this.mantissa * Math.pow(10, this.exponent))
                  + (other.sign * other.mantissa * Math.pow(10, other.exponent));
    ...
    subtract(other) {
        ...
        const adjThis = this.sign * this.mantissa * Math.pow(10, this.exponent - commonExp);
        const adjOther = other.sign * other.mantissa * Math.pow(10, other.exponent - commonExp);
        const diff = adjThis - adjOther;
        return new LargeNumber(diff);
    }

    // Raises the number to a floating point power.
    pow(exp) {
        if (exp instanceof LargeNumber) {
            exp = exp.toNumber(); // Optional: If exp is very large, implement its `.toNumber()` safely
        } else {
            exp = Number(exp);
        }
    
        // Special cases
        if (exp === 0) return new LargeNumber(1);
        if (this.sign === 0 || this.mantissa === 0) return new LargeNumber(0);
        ...
        // Negative exponents
        if (exp < 0) {
            const reciprocal = this.pow(-exp);
            return new LargeNumber(1).divide(reciprocal);
        }
    
        // Use scientific rule: (a * 10^b)^c = a^c * 10^(b * c)
        ...
        result.mantissa = newMantissa;
        result.exponent = newExponent;
        result.sign = Math.pow(this.sign, exp);
        return result;
    }
    ...
    floor() {
        let num = new LargeNumber(this.toString());
        num.mantissa = Math.floor(num.mantissa);
        num.exponent = Math.floor(num.exponent);
        return num;
    }
    
    ceil() {
        let num = new LargeNumber(this.toString());
        num.mantissa = Math.ceil(num.mantissa);
        num.exponent = Math.ceil(num.exponent);
        return num;
    }
...
class ExtremelyLargeNumber extends DerivedNumberType {
    ...
    constructor(n) {
        ...
        // Handle string input with custom "^^" exponential tower notation.
            ...
            if (parts.length === 1) {
                this.layer = 0;
                this.value = new LargeNumber(str);
            } else {
            ...
            if (negative) {
                this.value.sign = -Math.abs(this.value.sign);
            }
            this.tryToLayer();
        }
...
class StupendouslyLargeNumber extends DerivedNumberType {
    constructor(n) {
        super(0); // Initialize parent
        let negative = false; // <-- Added flag
        if (typeof n === 'number') {
            this.ultraLayer = 0;
            this.value = new ExtremelyLargeNumber(n);
        } else if (typeof n === 'string') {
            if(n[0] === '-') {           // <-- Detect leading '-'
                negative = true;
                n = n.slice(1);
            }
            if (n.startsWith("10##")) {
            ...
            if (negative) {               // <-- Force negative sign
                this.value.value.sign = -Math.abs(this.value.value.sign);
            }
