...
globalThis.DDCInjector = {
  ...
  injectScript: (name = DDCInjector.config.name, link, async = DDCInjector.config.async, module = false) => {
  ...
  injectStyle: (name = DDCInjector.config.name, link) => {
    ...
    style.onload = () => console.log(`Successfully loaded: (${name})`);
    style.onerror = (e) => {
