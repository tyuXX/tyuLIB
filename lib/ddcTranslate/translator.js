...
class Translator {
    ...
    translatePage(language) {
        document.querySelectorAll("[data-ddctranslate]").forEach(element => {
            if(this.translate(element.getAttribute("data-ddctranslate"), language) != element.getAttribute("data-ddctranslate"))
            {
                element.innerText = this.translate(element.getAttribute("data-ddctranslate"), language);
            }
            else{
                element.innerText = this.translate(element.getAttribute("data-ddctranslate").toLowerCase(), language);
            }
...
class Language {
    ...
    normalizeDictionary() {
        ...
        for (const key in this.dictionary) {
            newDict[key.toLowerCase()] = this.dictionary[key];
        }
        this.dictionary = newDict;
        // Convert all values to lowercase.
        for (const key in this.dictionary) {
            this.dictionary[key] = this.dictionary[key].toLowerCase();
        }
    }
    ...
    getTranslation(key) {
        if(this.hasCase) {
            ...
            if(key[0] === key[0].toUpperCase()) {
                return (this.dictionary[key][0].toUpperCase() + this.dictionary[key].slice(1)) ||key;
            }
        }
...
export { Translator, Language, activeLanguage, setActiveLanguage };