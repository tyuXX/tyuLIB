...
function loadStringSave(str) {
  let saveT = checkSaveIntegrity(str);
  let save = saveT.save;
  if (!saveT.success) {
    return null;
  }
...
function loadLocalSave(saveID) {
  ...
  let saveT = checkSaveIntegrity(localStorage.getItem("DDCSave-" + saveID));
  let save = saveT.save;
  if (!saveT.success) {
    return null;
  }
...
export { DDCSave, loadLocalSave, newSave, checkSaveIntegrity, loadStringSave };
export default { DDCSave, loadLocalSave, loadStringSave, newSave };