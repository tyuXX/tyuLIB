...
function newPage(name, id, html) {
    ...
    pages.push({id: id, name: name}); // Store the page ID and name
...
function removePage(id) {
    if(currentPage === id) {
        console.error("Cannot remove the active page!");
        return false; // Cannot remove the current page
    }
    if (!pages.some(page => page.id === id)) {
    ...
    pages = pages.filter(page => page.id !== id); // Remove the page from the list
}
...
function changePage(id) {
    ...
    // Fix: Check if a page with the provided id exists
    if (!pages.some(page => page.id === id)) {
        return false; // Page does not exist
    }
    if (currentPage !== "") {
